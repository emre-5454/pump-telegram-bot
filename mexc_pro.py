...
BOT_NAME = "MEXC PRO PARA GIRISI BOT"
...
def para_girisi_radar(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    if rsi_turn:
        score += 1; reasons.append("RSI toparlaniyor")
    if funding_ok:
...
def gold_long(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    if ema200_ok:
        score += 1; reasons.append("EMA200 ustu")
    if body_ok:
...
def flash_sweep(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    df5 = fetch_df(symbol, "5m", 120)
    df15 = fetch_df(symbol, "15m", 150)
    ...
    wick_5m = m5.lower_wick
    wick_15m = m15.lower_wick

    recovery_5m = m5.recovery_ratio
    recovery_15m = m15.recovery_ratio
    ...
    swept_low = m5.low <= recent_low_5m * 1.003 or m15.low <= recent_low_15m * 1.003
    ...
    if wick_5m >= 0.55 or wick_15m >= 0.55:
        score += 4; reasons.append("Buyuk alt igne")
    if recovery_5m >= 0.50 or recovery_15m >= 0.50:
        score += 3; reasons.append("Igneden toparlanma")
    if vol_ratio_5m >= 4.0 or vol_ratio_15m >= 3.0:
        score += 3; reasons.append("Hacim patlamasi")
    if usdt_vol_5m >= 30000 or usdt_vol_15m >= 50000:
        score += 2; reasons.append("USDT hacim guclu")
    if swept_low:
    ...
    if obv_turn:
        score += 1; reasons.append("OBV tepki")
    if macd_turn:
        score += 1; reasons.append("MACD toparlaniyor")

    valid = (
        score >= 15
        and (wick_5m >= 0.55 or wick_15m >= 0.55)
        and (recovery_5m >= 0.50 or recovery_15m >= 0.50)
        and (vol_ratio_5m >= 4.0 or vol_ratio_15m >= 3.0)
        and (usdt_vol_5m >= 30000 or usdt_vol_15m >= 50000)
        and bounce >= 3
    )
    ...
    return valid, {
        ...
        "dip_price": dip_price,
        "bounce": bounce,
        "vol_ratio": max(vol_ratio_5m, vol_ratio_15m),
        "usdt_vol": max(usdt_vol_5m, usdt_vol_15m),
        "lower_wick": max(wick_5m, wick_15m),
        "recovery": max(recovery_5m, recovery_15m),
        "reasons": reasons,
        "btc": btc_text,
        "funding_rate": funding_rate,
        "funding_text": funding_text
...
def safe_short(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    if rsi_ok:
        score += 1; reasons.append("RSI short icin uygun")
    if not btc_ok:
...
def analyze(item, btc_ok, btc_text):
    ...
    try:
        checks = [
            ("FLOW", COOLDOWN_FLOW, para_girisi_radar, "PARA GIRISI RADAR", "Takibe al. Para girisi var; 5m/15m kapanis takip."),
            ("GOLD", COOLDOWN_GOLD, gold_long, "GOLD LONG", "En kaliteli long takip. FOMO degil; retest bekle."),
            ("FLASH", COOLDOWN_FLASH, flash_sweep, "FLASH SWEEP RADAR", "Igne alarmi. Direkt long degil; kapanis takip."),
...
@app.route("/")
def home():
    return "MEXC PARA GIRISI BOT AKTIF", 200
