...
BOT_NAME = "🚄 MEXC PRO BOT | EARLY + GOLD + SWEEP + SQUEEZE + SHORT"
...
def early_radar(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    if ema_ok:
        score += 2; reasons.append("EMA21 üstü")
    if btc_ok:
    ...
    return valid, {
        "score": score, "price": m15.close, "rs": rs, "dist_low": dist_low,
        "vol_ratio": vol_ratio, "usdt_vol": usdt_vol, "rsi": h1.rsi,
...
def gold_long(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    if usdt_vol >= 20000:
        score += 1; reasons.append("USDT hacim yeterli")
    if change_3m >= 0.25:
        score += 2; reasons.append("3m momentum")
    if trend_up:
    ...
    valid = (
        score >= 13 and rs >= 70 and vol_ratio >= 2.2 and usdt_vol >= 20000
        and change_3m >= 0.20 and trend_up and macd_up and obv_up and body_ok and wick_ok
    )

    return valid, {
        "score": score, "price": m1.close, "rs": rs, "dist_low": dist_low,
        "vol_ratio": vol_ratio, "usdt_vol": usdt_vol, "change_3m": change_3m,
        "rsi15": m15.rsi, "body": m5.body_ratio, "upper_wick": m5.upper_wick,
        "reasons": reasons, "btc": btc_text,
        "funding_rate": funding_rate, "funding_text": funding_text
    }
...
def sweep_msb_long(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    if bb_touch:
        score += 2; reasons.append("Alt Bollinger yakın")
    if vol_ratio >= 2:
        score += 2; reasons.append("15m hacim patlaması")
    if usdt_vol >= 15000:
        score += 1; reasons.append("USDT hacim yeterli")
    if strong_wick:
        score += 3; reasons.append("Aşağı iğne")
    if recovered:
    ...
    valid = (
        score >= 15 and swept_low and reclaimed and msb and vol_ratio >= 2
        and strong_wick and recovered and obv_turn
    )

    return valid, {
        "score": score, "price": m15.close, "rs": rs, "dist_low": dist_low,
        "vol_ratio": vol_ratio, "usdt_vol": usdt_vol, "rsi": h1.rsi,
        "lower_wick": m15.lower_wick, "recovery": m15.recovery_ratio,
        "swept_low": swept_low, "reclaimed": reclaimed, "msb": msb,
        "reasons": reasons, "btc": btc_text,
        "funding_rate": funding_rate, "funding_text": funding_text
    }
...
def short_squeeze_long(symbol, rs, dist_low, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    if strong_green:
        score += 3; reasons.append("Güçlü yeşil dönüş")
    if vol_ratio >= 2:
        score += 2; reasons.append("15m hacim patlaması")
    if usdt_vol >= 15000:
        score += 1; reasons.append("USDT hacim yeterli")
    ...
    valid = (
        score >= 16 and recent_breakdown and reclaim_ema21 and strong_green
        and vol_ratio >= 2 and obv_boom and macd_turn and rsi_turn and msb
    )

    return valid, {
        "score": score, "price": m15.close, "rs": rs, "dist_low": dist_low,
        "vol_ratio": vol_ratio, "usdt_vol": usdt_vol, "rsi": h1.rsi,
        "body": m15.body_ratio, "recent_breakdown": recent_breakdown,
        "reclaim_ema21": reclaim_ema21, "msb": msb,
        "reasons": reasons, "btc": btc_text,
        "funding_rate": funding_rate, "funding_text": funding_text
    }
...
def dump_short(symbol, rs, btc_ok, btc_text, funding_rate, funding_text, funding_ok):
    ...
    return valid, {
        ...
        "vol_ratio": vol_ratio, "usdt_vol": usdt_vol, "rsi": h1.rsi,
        "body": m15.body_ratio, "breakdown": breakdown,
        "trend_down": trend_down, "macd_down": macd_down, "obv_down": obv_down,
        "reasons": reasons, "btc": btc_text,
        "funding_rate": funding_rate, "funding_text": funding_text
    }
...
Hacim Artışı:
{d.get('vol_ratio', 0):.2f}x

USDT Hacim:
{int(d.get('usdt_vol', 0))} USDT
...
def analyze(item, btc_ok, btc_text):
    ...
    try:
        checks = [
            ("SQUEEZE", COOLDOWN_SQUEEZE, short_squeeze_long, "🟢 SHORT SQUEEZE LONG", "Short squeeze / fake dump dönüş adayı. 5m retest bekle."),
            ("SWEEP", COOLDOWN_SWEEP, sweep_msb_long, "🧲 SWEEP + MSB LONG", "Dipten dönüş adayı. Direkt long değil; tutunma bekle."),
            ("GOLD", COOLDOWN_GOLD, gold_long, "🏆 GOLD LONG", "En kaliteli long takip. FOMO değil; 5m retest bekle."),
            ("EARLY", COOLDOWN_EARLY, early_radar, "👀 EARLY RADAR", "Takibe al. Kırılım gelmeden direkt long değil."),
            ("SHORT", COOLDOWN_SHORT, dump_short, "🔻 DUMP / SAFE SHORT", "Short adayı. Retest bekle.")
        ]
...
def run_bot():
    send_telegram(f"✅ {BOT_NAME} başladı. MEXC PRO aktif.")
    ...
    while True:
        try:
            ...
            send_telegram(f"⚠️ MEXC bot genel hata:\n{e}")
            time.sleep(30)
