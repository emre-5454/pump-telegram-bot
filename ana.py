...
def score_signal(df):
    ...
    if usdt_volume_15m < MIN_15M_VOLUME_USDT:
        return 0, []
...
def scan_exchange(exchange_name):
    ...
    print(
        f"{exchange_name.upper()} toplam coin: {len(symbols)}",
        flush=True
    )
    ...
    for symbol in symbols:
        ...
        try:
            ...
            if (
                key in last_alert
                and now - last_alert[key] < COOLDOWN_SECONDS
            ):
                continue
...
def main():
    ...
    send_telegram(
        "✅ Railway MEXC elit filtre botu başladı."
    )
